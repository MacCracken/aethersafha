...
| `AE-7` | Pointer input | ⛔ **not possible today** — agnos xhci matches only HID boot **keyboard**, protocol 0x01. ⚠ `AE-M` below removes the *stimulus* argument for deferring `AE-0a`'s proof, but not the need for a pointer |
