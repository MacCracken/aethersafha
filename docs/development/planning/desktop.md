...
Earlier: ⭐⭐⭐ **the shader composites the desktop's surfaces AND its text, on iron.**
`AE-6` (`#92` op 0x01) and `AE-8` (`#92` op 0x03 GLYPH_1BPP) both burned PASS with no fallback line, 673 frames
([card](https://github.com/MacCracken/agnosticos/blob/main/docs/development/iron-nuc-zen-log.md#tracker-15641-ae6-ae8)).
Earlier the same day, on the same byte-identical kernel: **BURNED AND PASSED (AGNOS 1.56.41, `smp: cpus online: 4`).** The `#97` channel band runs
...
- ✅ **`AE-0a` — no ghosting** across 278 frames, on the only substrate that could show one. ⚠ An absence,
  observed once, by eye, with **static** windows. ⭐ The moving case is now exercised in QEMU on the CPU path
  (`AE-M`, 2026-08-08: a window moved 40 px left and 40 px down, no ghost); on the **GPU** path it is still open.
...
| `AE-7` | Pointer input (USB mouse) | ⭐ **DESIGNED 2026-08-08; P0 DONE, P1-P4 unbuilt** — full design in agnos [`planning/pointer.md`](https://github.com/MacCracken/agnos/blob/main/docs/development/planning/pointer.md); read it before writing any pointer code. ⛔ The old note here ("not possible — xhci matches only protocol 0x01") was **too pessimistic**: enumeration already addresses the mouse, and `SETU_INPUT_PTR_MOVE`/`_BTN` (kinds 9/10) already exist with live call sites, so **no protocol change is needed**. The real work is four defects a keyboard cannot expose — `hid_poll` silently eating unmatched events, its keyboard-only gate, one shared report buffer that makes coalesced *relative* deltas **amplify** ~2-3x, and this repo's own chrome-inside vs chrome-outside rect disagreement that would put every click 30 px out. ⛔ A pointer event must NOT ride the scancode ring: `dX = 0x01` decodes to HID 0x29 = Esc, so moving the mouse would QUIT the desktop — and an untagged pointer event in the `events[]` batch does the same one layer up (`dY` 41 = Esc). A kind tag is mandatory. ⚠ The cursor composites via `draw_text`'s queue (drained AFTER client blits), and needs a retired rect for the same reason a closed window does |
| `AE-M` | Keyboard window mover (F7-F10 slide the focused window 40 px) | ⭐ **QEMU-PROVEN 2026-08-08, unburned.** The stimulus `AE-0a` never had. Eleven injected keys produced eleven usage traces in exactly the right order (`43, 64,64,64, 65,65, 66,66, 67,67,67`) on `qemu-xhci` + `usb-kbd` — **the same USB HID producer as archaemenid** — and the window moved by the exact net of the sequence with **no ghost**. Mapping lives in `input_map`/`input_apply`, so it is unit-tested (`tests/input.tcyr` 15 → **44 asserts**, two mutants confirm they bite). ⛔ **The first cut of this was DEAD CODE and flew to hardware that way** — nested inside the TAB branch, so `usage >= 0x3A` sat inside `usage == 0x2B`; see the falsified list. ⛔ **F7-F10, not F1-F4**: F4 is `IA_CLOSE_FOCUSED`, so the first binding made "move down" and "destroy the window" one key |
...
| `AE-8` | Glyphs off the CPU (`#92` op 0x03 GLYPH_1BPP) | ⭐⭐⭐ **BURNED PASS 2026-08-07** — `chrome text is on the shader cores`, and the panel shows readable titlebar captions, which is the check that matters. No refusal line. **SHIPPED.** Chrome text is queued during render and emitted by `ae_gpu_emit_text()` after the `#39` chrome blit and the client surfaces, before the flip — a glyph op issued during render would be erased by that blit. ⭐ The format already matched: kashi's VGA 8x16 is 1bpp, one byte per row, `bit (7 - col)` leftmost, and the kernel's shader reads **MSB-first**, so the packer copies bits at an x-offset rather than converting. One `#86` slot allocated once and reused (16 slots system-wide, each rounded to a 2 MB page). ⛔ Refusals (queue/pool full, over-long run, no slot) fall back to the per-pixel CPU path and are COUNTED — text is never lost, and a glyph failure never demotes the frame. **Verified bit-exactly on the host with no GPU**: the packed mask expands to the pixels `draw_text` produces, **0 differing pixels**, with a negative control that reads the mask LSB-first (the mirroring mistake the kernel's own source names) and must fail. 99/99 in `render.tcyr`. ⚠ QEMU takes the CPU branch, so the desktop run there verifies the FALLBACK; the shader path runs on iron |
| `AE-9` | `#88 gpu_fill_rect` for the chrome + the clear; `#39` DELETED. (⭐ `#89` bytes +4..+31 now CONSUMED — see below; `#91` and batched `#92` are NOT reachable — falsified list) | ⭐⭐⭐ **BURNED PASS 2026-08-08** — `chrome fills are on the GPU … the #39 blit is gone` in BOTH runs of the compositor, no `refused` line. **The CPU no longer draws the desktop.** ⭐ Bonus: run 2 in the same boot came up clean, the first hardware evidence that the band has no port to leak (0.12.0's leaked 7700 made run 2 host nothing while looking healthy). ⚠ No speed claim — 92/142 frames vs 673 is an early quit, and no burn in the series measured frame duration. **SHIPPED 2026-08-07 — the `#39` chrome blit is no longer issued on a GPU frame.** With the clear, the chrome rects, the glyphs and the client surfaces all on the GPU, nothing is left in the userland framebuffer to copy. Exactly the order `gpu.cyr`'s `#85`-removal note predicted. ⭐ The clear stops being a CPU store loop entirely — the `AE-0a` band is just the first rect in the queue. ⛔ **All-or-nothing per frame** (`ae_chrome_on_gpu()`): a half-migrated frame has `#39` and `#88` overwriting each other. ⛔ **Clipping moved into the queue** because `#88` REFUSES an out-of-range rect where `fill_rect` clipped silently — otherwise a window dragged one pixel off-screen would retire the path. ⛔ A refusal still blits THIS frame (the framebuffer holds no chrome, so neither-fill-nor-blit shows nothing) and latches to the CPU after. 134/134 in `render.tcyr`. ⚠ **Batched `#92` is NOT possible as the ladder assumed** — see the falsified list. ⚠ QEMU proves the fallback; the GPU path runs on iron |
...
- ⛔⛔ **"The compositor's window mover works because it compiles and its string is in the binary" is FALSE, and
  ...
  least 58. Two mutually exclusive predicates on the same pure expression, inner lexically inside outer:
  **unreachable for every possible input.** It compiled, it linked, `strings` found its console text in the
  flashed image, and the only symptom on real hardware was one absent log line. ⇒ Three lessons, all paid for:
  **(a)** `strings` proves a literal was COMPILED, never that it is REACHABLE — do not use it as a staging gate
  for behaviour. **(b)** An inline block inside a 250-line frame loop is untestable *by construction*; moving the
  mapping into `input_map`/`input_apply` took the suite from 15 to 44 asserts on the same logic. **(c)** In Cyrius
  the giveaway was **indentation disagreeing with brace depth** — count braces mechanically (strip comments
  first), and remember there is no chained else-if to make the nesting obvious.
- ⛔ **"A missing log line on iron means the operator did not press the key" is NOT a permitted default.** From
  that 2026-08-08 log, "never pressed" and "arrived and was silently dropped" were **indistinguishable**, because
  the only marker that could separate them was itself inside the dead block. The mechanism turned out to be a
  bug in this repo's own source. ⇒ The compositor now names the first twelve key usages it sees, always on, so
  the question is answerable from any boot's log. A burn that cannot distinguish its own failure modes cannot be
  re-run into an answer, only re-run into the same ambiguity.
- ⛔ **"agnos filters keys with no ASCII mapping, so F-keys cannot reach a compositor" is WRONG for this path.**
  The filter is real — `read#5`/`scancode_to_ascii` genuinely drops every key that cooks to 0 — but the
  compositor uses **`kbscan#42`**, a pure byte copy of raw Set-1 make+break codes with no keymap and no ASCII
  step, and `lib/bhumi.cyr:699` maps Set-1 `0x3B-0x44` → HID `0x3A-0x43` as one range (F1-F10 all decode).
  ⚠ Naming the wrong path would have converted a one-line source bug into a phantom kernel defect. ⚠ Both paths
  drain the same `kb_buf`, so a shell and a compositor in one boot compete for keystrokes.
- ⛔ **"Correlate two screendumps for a rigid shift to measure that a window moved" DOES NOT WORK, and returns a
  confident wrong answer.** Only ONE window moves, so every static pixel of console and of the other windows
  votes for zero displacement and the best match is always (0,0) — the instrument reported "nothing moved" about
  a frame in which a window had demonstrably moved 40 px. ⇒ Rendering both frames and **looking** settled it in
  one glance. A whole-frame measure cannot answer a per-object question.
...
- ⛔⛔ **`#91 gpu_blit_bb` HAS NO CORRECT CONSUMER IN THIS COMPOSITOR (found 2026-08-08).** The ladder listed
  it as a window-move optimisation. **Every window here is a live client surface re-composited every frame by
  design** — `render_window` re-reads a `bufid` window's buffer each frame and the damage model marks such
  windows damaged unconditionally, because *"treating those as undamaged would freeze animating windows"*. So
  the pixels a `#91` copy would move are being rewritten from the client's buffer anyway; the copy saves
  nothing, and for a chrome-only window `#88` is already cheaper. ⚠ Secondary obstacle: `#84 present` FLIPS,
  so a copy's source is 1 or 2 frames stale depending on a buffer parity **ring 3 cannot observe**
  (`gpu_bb_back` is kernel-private and absent from the `#89` record) — derivable by counting presents, so the
  content argument is the load-bearing one. ⇒ It becomes useful only under a compositor that PRESERVES the
  back buffer and touches only damage, which is a different frame architecture from the one `AE-9` finished.
- ⛔⛔ **BATCHED `#92` (64 RECORDS/SUBMISSION) IS NOT REACHABLE FOR GLYPHS, AND THE LADDER ASSUMED IT WAS
  (2026-08-07).** A batch is handed over as a block and processed after submission, and every record names
  its own `mask_id` slot — so N glyph runs sharing ONE staging slot would all read the LAST run's mask. The
  glyph path reuses a single slot deliberately: `#86` slots are scarce (16 system-wide, each request rounded
  up to a 2 MB page), so a slot per run would exhaust the table. ⇒ Batching needs **N slots** or a
  per-record source-offset field the ABI does not have. It is not a small follow-up to `AE-9`; it is a slot
  model or an ABI extension. The chrome fills use `#88`, a plain syscall, so batching never applied to them.
