...
> **STATUS 2026-08-17: E ✅, C ✅ (rupa 0.1.3 + dhancha 0.9.6), A ✅ and B ✅ (dhancha 0.9.7). D is
> unblocked.** ⚠ D is now the rung that MATTERS: A and B are only claims until a consumer deletes its
> hand-rolled equivalent. crab's pane and puka's buffer are the two lists `dh_list_*` was written from,
> and a port that keeps them is evidence the container is wrong, not evidence the port is incomplete.
