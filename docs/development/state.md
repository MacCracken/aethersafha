...
| `--agnos` build | **GREEN** — 0.16.2 is **4,075,560 B · `e6789468`**. ⛔⛔ **~10 MB SMALLER THAN THE STAGED TOOL AND NOBODY KNOWS WHY YET.** `build/rootfs/bin/aethersafha` is **14,069,784 B**; the difference is `.bss` **11,545,776 → 1,551,568** with `.text` unchanged (2,472,800 → 2,473,648), so it is static data, not code. It is **NOT the compiler** (6.5.20 and 6.5.21 build identically) and it predates this cut — it opened between the last staging and the previous session's commits. `sigil` declares ~10.08 MB of module-scope arrays, matching the delta; that is where to start. **Unattributed, deliberately: do not restage on the assumption it is benign.** ⛔ **A SIZE DOES NOT IDENTIFY A BINARY** — 2026-08-12 three distinct artifacts shared one byte count (`1b8b3c57` · staged `690160b9` · rebuild `a692279f`). Quote a hash beside every size | `sha256sum` |
...
⭐⭐ **M6-C3 PER-WINDOW OPACITY IS IRON-PROVEN (2026-08-16, agnos 1.56.44).** `#89 +28` reported `0x1FF5F`, `ae_gpu_alpha_ok()` answered 1, and `#92` **op 0x06** composited a translucent client surface across 244 and 537 frames — no `GPO_E_*`, no demote, `frame loop ok`. ⛔ **The same burn found a regression of mine**: at `--opacity 128` every window lost its titlebar background, because `fill_rect_a` has no `#88` route and the frame still skipped the `#39` blit. Fixed by `ae_gpu_alpha_chrome` — a translucent frame puts chrome/glyphs/backdrop on the CPU and keeps the CLIENT SURFACES on the GPU. ⚠ **Not settled by that burn**: the rounding tie (agnos owns it), window OVERLAP (one translucent over one opaque is not an overlap, so the z-order fix is still host-only evidence), and a NON-premultiplied surface, which is composited opaque at any opacity on both paths.
