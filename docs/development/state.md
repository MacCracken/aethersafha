...
| Field | Value | Source |
|---|---|---|
| Version | **0.16.12** (2026-08-19) | [`VERSION`](../../VERSION) |
...
| Modules / tests | 28 `src/*.cyr` · **25 `.tcyr` suites, 1,732 assertions** | `ls` · `cyrius test` |
| `--agnos` build | **GREEN** — 0.16.12 is **4,112,032 B · `5cda77fd`**. ⛔ **A SIZE DOES NOT IDENTIFY A BINARY**: 0.16.10 and 0.16.11 both measured 4,107,920 B under different hashes (`4e8ba754`, `82633487`), and 0.16.12 built with and without the render fix measured the SAME 4,112,032 B under different hashes (`eaadef21`, `5cda77fd`). Quote a hash beside every size. ⚠ The whole **+4,112 B** is the 6.5.28 stdlib, not compositor code — `lib/freelist.cyr` gained redzone poisoning. An earlier note here called the 6.5.27 -> 6.5.28 bump byte-identical "measured, not assumed"; that measurement ran BEFORE `lib/` was re-vendored, so it compared two builds against the same old stdlib and measured nothing. Re-vendor first, then measure | `sha256sum` |
| Host (Linux) build | **GREEN**, 25/25 suites (1,732 assertions). ⭐ `--clients` reaches **exit 95** with two clients since **0.13.1** — it was structurally unreachable before (`running = 0` on first present). ⭐ 0.13.2 adds a **latched scanout-refused report**, negative-controlled: with `/dev/fb0` hidden it prints `drawing to nothing / -1`, with it present it is silent | measured |
