...
| Field | Value | Source |
|---|---|---|
| Version | **0.13.7** | [`VERSION`](../../VERSION) |
...
| Backend | **bhumi 1.2.1** — ⭐⭐ **SCANOUT IS LIVE ON LINUX** (fbdev; bhumi ADR 0003). ⛔ 1.2.0's arm used `mmap` and displayed NOTHING on shadow-buffer fbdevs (`simpledrm`); 1.2.1 uses `pwrite`. **Found in QEMU — the dev box's amdgpu could not show it.** ⭐⭐ **KEYBOARD + POINTER LIVE on Linux** (bhumi 1.4.0, evdev, one shared drain; QEMU-proven — cursor moved by exactly the injected delta, click routed, Esc quit). ⛔ Base plane only: evdev emits no 0xE0 prefix, so arrows/RCtrl/Meta/Home/End/PgUp/PgDn/Insert/Delete are dropped. GPU via the agnos ring-3 band `#82`-`#94` — **not** mabda; there is no `[deps.mabda]` | `bhumi/src/scanout.cyr` |
